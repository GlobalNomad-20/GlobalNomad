...
export const TextareaInput = ({ name, label, placeholder, required, className }: InputProps) => {
  const {
    register,
    formState: { errors },
  ...
  return (
    <InputWrapper label={label} error={error} required={required}>
      <textarea
        ...
        className={cn(
          "typo-16-m h-35 w-full rounded-2xl border border-gray-100 px-5 py-4.5 transition-colors",
